...
int File_Open(const char *pFilename, uint32 openFlags)
{
	...
	gOpenFiles[fileID].file = CreateFile(pXFilename, access, FILE_SHARE_READ, NULL, (openFlags&OF_Write) ? OPEN_ALWAYS : OPEN_EXISTING, NULL, NULL);
...
uint32 File_GetSize(uint32 fileHandle)
{
	...
	DWORD excess;
	DWORD fileSize = GetFileSize(gOpenFiles[fileHandle].file, &excess);

	DBGASSERT(excess == 0, "Fuji does not support files larger than 4,294,967,295 bytes.");

	gOpenFiles[fileHandle].len = fileSize;
