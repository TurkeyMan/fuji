...
void F3DFile::ExportMesh(char* &pData, char *pBase)
{
	...
	for(size_t a=0; a<meshChunk.subObjects.size(); a++)
	{
...
//		pMesh[a].triangleCount = (uint32)meshChunk.subObjects[a].triangles.size();
//		pMesh[a].vertexCount = (uint32)meshChunk.subObjects[a].vertices.size();
		pMesh[a].positionCount = (uint32)meshChunk.subObjects[a].positions.size();
		pMesh[a].texCount = (uint32)meshChunk.subObjects[a].uvs.size();
		pMesh[a].normalCount = (uint32)meshChunk.subObjects[a].normals.size();
		pMesh[a].colourCount = (uint32)meshChunk.subObjects[a].colours.size();
		pMesh[a].illumCount = (uint32)meshChunk.subObjects[a].illumination.size();
	}
	...
	for(size_t a=0; a<meshChunk.subObjects.size(); a++)
	{
		...
		for(uint32 b=0; b<pMesh[a].matSubCount; b++)
		{
			...
			pMatSubs[b].triangleCount = (uint32)meshChunk.subObjects[a].matSubobjects[b].triangles.size();
			pMatSubs[b].vertexCount = (uint32)meshChunk.subObjects[a].matSubobjects[b].vertices.size();
...
{
	size_t size = sizeof(F3DBone)*skeletonChunk.bones.size();
...
{
	size_t size = sizeof(F3DMaterial)*materialChunk.materials.size();
...
void WriteMeshChunk_Generic(F3DFile *pModel, MFMeshChunk *pMeshChunks, const F3DSubObject &sub, char *&pOffset, MFStringCache *pStringCache, MFBoundingVolume *pVolume)
{
	...
	for(size_t a=0; a<sub.matSubobjects.size(); a++)
	{
		if(sub.matSubobjects[a].triangles.size() != 0)
			numMeshChunks += (int)sub.matSubobjects[a].triangleBatches.size();
	}
	...
	for(size_t a=0; a<sub.matSubobjects.size(); a++)
	{
		...
		for(size_t b=0; b<matsub.triangleBatches.size(); b++)
		{
			const F3DBatch &batch = matsub.triangleBatches[b];

			int numTriangles = (int)batch.tris.size();
			int numIndices = numTriangles*3;
			int numVertices = (int)batch.vertices.size();
			...
			chunk.matrixBatchSize = (int)batch.bones.size();
...
void WriteMeshChunk_PSP(F3DFile *pModel, MFMeshChunk *pMeshChunks, const F3DSubObject &sub, char *&pOffset, MFStringCache *pStringCache, MFBoundingVolume *pVolume)
{
	...
	for(size_t a=0; a<sub.matSubobjects.size(); a++)
	{
		...
		for(size_t tb=0; tb<matsub.triangleBatches.size(); tb++)
		{
			const F3DBatch &batch = matsub.triangleBatches[tb];
...
void *F3DFile::CreateMDL(size_t *pSize, MFPlatform platform, size_t extraBytes)
{
	...
	for(size_t a=0; a<GetMeshChunk()->subObjects.size(); a++)
	{
...
//			pSubobject[b].pMaterial = (MFMaterial*)pStringCache->Add(materialChunk.materials[sub.materialIndex].name);
			...
			// count valid mesh chunks
			for(size_t c=0; c<sub.matSubobjects.size(); c++)
			{
	...
	if(skeletonChunkIndex > -1)
	{
		...
		size_t numBones = GetSkeletonChunk()->bones.size();
		...
		*pBoneRemappingTable = -1;
		...
		for(size_t a=0; a<numBones; a++)
		{
			...
			if(bone.bIsReferenced)
			{
				...
				pBoneChunk[bc].pParentName = bone.parent > -1 ? MFStringCache_Add(pStringCache, bone.parentName.CStr()) : NULL;
				...
				pBoneChunk[bc].parent = bone.parent > -1 ? (int16)pBoneRemappingTable[bone.parent] : -1;
	...
	if(collisionChunkIndex > -1)
	{
		...
		pDataHeaders[collisionChunkIndex].count = (int)GetCollisionChunk()->collisionObjects.size();

		pOffset += MFALIGN16(sizeof(MFCollisionTemplate)*pDataHeaders[collisionChunkIndex].count);

		for(int a=0; a<pDataHeaders[collisionChunkIndex].count; a++)
		{
			...
			switch(pCollisionChunk[a].type)
			{
				case MFCT_Mesh:
				{
					...
					pMesh->numTris = (int)pColMesh->tris.size();
	...
	if(tagChunkIndex > -1)
	{
		...
		pDataHeaders[tagChunkIndex].count = (int)GetRefPointChunk()->refPoints.size();
	...
	for(int a=0; a<pModelData->numDataChunks; a++)
	{
		switch(pModelData->pDataChunks[a].chunkType)
		{
			case MFChunkType_SubObjects:
			{
				...
				for(int b=0; b<pModelData->pDataChunks[a].count; b++)
				{
...
//					MFFixUp(pSubobjectChunk[b].pMaterial, pStringBase, 0);
	...
	void *pMDL = MFHeap_Alloc(fileSize + extraBytes);
...
void *F3DFile::CreateANM(size_t *pSize, MFPlatform platform, size_t extraBytes)
{
	if(!animationChunk.anims.size())
	{
		// no animation
	...
	for(uint32 a=0; a<pAnimData->numBones; a++)
	{
		...
		pOffset += MFALIGN16(sizeof(float)*bone.numFrames);
		...
		// fill kay array
		for(uint32 b=0; b<pAnimData->pBones[a].numFrames; b++)
		{
	...
	void *pANM = MFHeap_Alloc(fileSize + extraBytes);
...
void F3DFile::Optimise()
{
	...
	for(size_t a=0; a<GetMeshChunk()->subObjects.size(); a++)
	{
		...
		for(size_t b=0; b<sub.matSubobjects.size(); b++)
		{
			...
			for(size_t c=0; c<matsub.triangles.size(); c++)
			{
			...
			for(int c=0; c<(int)matsub.vertices.size(); c++)
			{
				if(vertexUsage[c])
				{
					if(x != c)
		...
		int posSize = (int)sub.positions.size();
		int uvSize = (int)sub.uvs.size();
		int normSize = (int)sub.normals.size();
		int colSize = (int)sub.colours.size();
		...
		for(int b=0; b<(int)sub.matSubobjects.size(); b++)
		{
			F3DMaterialSubobject &matsub = sub.matSubobjects[b];

			for(size_t c=0; c<matsub.vertices.size(); c++)
			{
		...
		for(size_t b=0; b<sub.matSubobjects.size(); b++)
		{
			...
			size_t vertSize = matsub.vertices.size();
...
void AddVertsToBatch(F3DBatch &batch, const F3DMaterialSubobject &matSub, const F3DTriangle &tri)
{
	...
	{
		batch.vertexMapping[v0] = (int)batch.vertices.size();
		batch.vertices.push(v0);
	}
	if(batch.vertexMapping[v1] == -1)
	{
		batch.vertexMapping[v1] = (int)batch.vertices.size();
		batch.vertices.push(v1);
	}
	if(batch.vertexMapping[v2] == -1)
	{
		batch.vertexMapping[v2] = (int)batch.vertices.size();
...
void AddTriToBatch(F3DBatch &batch, const F3DMaterialSubobject &matSub, const F3DTriangle &tri)
{
	...
	for(int a=0; a<matSub.maxWeights; a++)
	{
		...
		if(b0 > -1 && batch.boneMapping[b0] == -1)
		{
			batch.boneMapping[b0] = (int)batch.bones.size();
			batch.bones.push(b0);
		}
		if(b1 > -1 && batch.boneMapping[b1] == -1)
		{
			batch.boneMapping[b1] = (int)batch.bones.size();
			batch.bones.push(b1);
		}
		if(b2 > -1 && batch.boneMapping[b2] == -1)
		{
			batch.boneMapping[b2] = (int)batch.bones.size();
			batch.bones.push(b2);
		}
	}
}
...
void F3DFile::BuildBatches(MFPlatform platform)
{
	...
	for(size_t a=0; a<meshChunk.subObjects.size(); a++)
	{
		for(size_t b=0; b<meshChunk.subObjects[a].matSubobjects.size(); b++)
		{
...
*/
			{
				int numTris = (int)matSub.triangles.size();
				...
				while(numAdded < numTris)
				{
					...
					int numVerts = (int)matSub.vertices.size();
					int numBones = (int)skeletonChunk.bones.size();
					...
					for(int c=0; c<numTris; c++)
					{
						...
						if(numNotInBatch + (int)batch.bones.size() > maxBones)
							continue;
...
void F3DFile::ProcessCollisionData()
{
	...
	for(size_t i=0; i<GetMeshChunk()->subObjects.size(); i++)
	{
		...
		if(sub.name.EqualsInsensitive("c_"))
		{
			...
			for(size_t a=0; a<sub.matSubobjects.size(); a++)
			{
				triCount += (int)sub.matSubobjects[a].triangles.size();
			}
...
int F3DSubObject::IsSubobjectAnimation() const
{
	...
	for(size_t a=0; a<matSubobjects.size(); a++)
	{
...
{
	for(size_t a=0; a<collisionObjects.size(); a++)
	{
